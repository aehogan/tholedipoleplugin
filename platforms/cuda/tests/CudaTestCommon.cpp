...
// Get tolerance multiplier based on CUDA precision
static double getPrecisionMultiplier() {
    string precision = cudaPlatform->getPropertyDefaultValue("CudaPrecision");
    if (precision == "single")
...
void assertForcesAndEnergiesMatch(System& system, vector<Vec3>& positions,
    ...
    // Adjust tolerances based on CUDA precision
    ...
    for (int i = 0; i < system.getNumParticles(); i++) {
        cudaSystem.addParticle(system.getParticleMass(i));
    ...
    if (a[0] > 0) {
        cudaSystem.setDefaultPeriodicBoxVectors(a, b, c);
    ...
    TholeDipoleForce* cudaForce = cloneTholeDipoleForce(origForce);
    TholeDipoleForce* refForce = cloneTholeDipoleForce(origForce);

    cudaSystem.addForce(cudaForce);

    VerletIntegrator integCuda(1.0);
    VerletIntegrator integRef(1.0);

    // Create CUDA context first to get calculated PME parameters
    ...
    if (cudaForce->getNonbondedMethod() == TholeDipoleForce::PME) {
        double alpha;
        int nx, ny, nz;
        cudaForce->getPMEParametersInContext(cudaContext, alpha, nx, ny, nz);
    ...
    double cudaEnergy = cudaState.getPotentialEnergy();

    ASSERT_EQUAL_TOL(refEnergy, cudaEnergy, energyTol);

    int numParticles = cudaSystem.getNumParticles();
    for (int i = 0; i < numParticles; i++) {
        for (int j = 0; j < 3; j++) {
            ASSERT_EQUAL_TOL(refState.getForces()[i][j], cudaState.getForces()[i][j], forceTol);
        }
    }
}
